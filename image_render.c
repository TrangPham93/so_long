...
/*   Updated: 2025/02/20 19:52:32 by trpham           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_background(t_data *data, t_game *game)
{
	...
	// mlx_put_image_to_window(data->mlx_ptr, data->mlx_win, data->img_background, 32, 0);
}
...
void	load_images(t_data *data)
{
	...
	data->img_wall = mlx_xpm_file_to_image(data->mlx_ptr, "./images/wall.xpm",	&img_width, &img_height);
	if (!data->img_wall)
	{
		mlx_destroy_image(data->mlx_ptr, data->img_wall);
		handle_error("Failed to load image");
	}
	data->img_player = mlx_xpm_file_to_image(data->mlx_ptr, "./images/cow.xpm",	&img_width, &img_height);
	if (!data->img_player)
	{
		mlx_destroy_image(data->mlx_ptr, data->img_player);
		handle_error("Failed to load image");
	}
	data->img_exit = mlx_xpm_file_to_image(data->mlx_ptr, "./images/exit.xpm",	&img_width, &img_height);
	if (!data->img_exit)
	{
		mlx_destroy_image(data->mlx_ptr, data->img_exit);
		handle_error("Failed to load image");
	}
	data->img_collects = mlx_xpm_file_to_image(data->mlx_ptr, "./images/milk.xpm",	&img_width, &img_height);
